...
class Relic < ActiveRecord::Base
  ...
  def relic_to_csv(register = false)
    CSV.generate do |csv|
      csv << [I18n.t("activerecord.models.relic.one")]
      self.append_csv(csv, register)

      csv << []
      csv << [I18n.t("activerecord.models.subrelic.one").capitalize]
      descendants.each do |subrelic|
        subrelic.append_csv(csv, register)
        csv << []
      end
    end
  end

  def append_csv(csv, register)
    ...
    if place 
      ...
      csv << [place.name, commune_nm, district_nm, voivodeship_nm]
    end
  end
