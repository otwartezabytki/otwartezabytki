...
class Widget::Timeline < Widget
  ...
  def snippet
    widget_url = Rails.application.routes.url_helpers.widgets_timeline_url(uid, :host => Settings.oz.host)
    params = api_params.blank? || api_params.empty? ? "" : "?#{ActiveSupport::JSON.decode(api_params).to_query(:search)}" rescue ""
    "<iframe id='oz_timeline' src='#{widget_url}#{params.html_safe}' width='#{width}' height='#{height}'></iframe>"
  end
